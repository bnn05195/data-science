...
* **메인 막대그래프 (히스토그램):** 가로축(운송 거리)을 일정 구간으로 나누어, 각 거리 구간마다 트럭에 실린 화물이 얼마나 되는지 쌓아 올린 그래프입니다. 막대가 높게 솟은 곳이 바로 트럭 운송 수요가 가장 빵빵하게 몰려 있는 '최적 거리'입니다. 거리가 너무 멀어지면 철도 등 다른 운송 수단에 밀려 막대가 낮아지는 것을 볼 수 있습니다.
* **위쪽 상자 모양 (박스 플롯, Box Plot):** 전체 물동량 데이터가 어떻게 흩어져 있는지 한눈에 보여주는 요약본입니다. 가운데 네모난 상자 부분이 전체 트럭 화물 이동의 절반(50%)이 집중된 핵심 거리 구간을 뜻하며, 양옆으로 뻗은 선(수염)은 나머지 화물들이 어디까지 퍼져서 이동하는지 대략적인 최대/최소 범위를 나타냅니다.
""")